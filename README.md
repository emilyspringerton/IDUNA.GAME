...
A real fork of [PITVIPER](../PITVIPER) (SDL2 terminal emulator with Emily Prime integration
hooks) — reskinned into Solarized Light and given a text-based prompt sequence that shows IDUNA's
real honor code and walks a user through IDUNA's real device-auth login flow, all rendered as
terminal text in IDUNA's own brand palette. Launched as a third app from SHANKPIT OS's lobby
grid. See `NORTHSTAR.md` for the full real investigation (auth-namespace landscape, why device
auth and not embedded Google OAuth, what's built vs. named-not-solved).

## Why a fork, not a rewrite

Same "new fork, not rewrite-in-place" precedent SAND already set against PITVIPER: PITVIPER's own
real engine (PTY/SSH/MUD connections, GPU glyph atlas, vterm) is kept exactly as-is here, not
rebuilt. This repo only changes the palette (`baseColors`/`idunaPalette` in `cmd/idunagame/
main.go`) and adds one new thing on top (`cmd/idunagame/iduna_prompt.go`, `internal/reflux/`).
PITVIPER's own repo is unaffected.
...
IDUNA_HOST=127.0.0.1 IDUNA_PORT=8080 ./idunagame        # env vars point at a real IDUNA instance
```

IDUNA.GAME is not part of the root `go.work` workspace — the Makefile always builds with
`GOWORK=off`. If you build manually, do the same:
`GOWORK=off CGO_ENABLED=1 go build -o idunagame ./cmd/idunagame`.
