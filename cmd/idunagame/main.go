...
// IDUNA.GAME — a real fork of PITVIPER (SDL2 terminal emulator with Emily Prime integration),
// reskinned into Solarized Light (NORTHSTAR.md §1, founder real-time: "have it be a reskin of
// pitviper into solarized light with a text based prompt system"). PITVIPER's own real engine
// (PTY/SSH/MUD connections, vterm, the GPU glyph atlas below) is kept as-is, not rewritten —
// same "new fork, not rewrite-in-place" precedent SAND already set against PITVIPER itself. The
// real, new thing this fork adds is an IDUNA-branded text-prompt startup sequence (see
// runIdunaPrompt in iduna_prompt.go) that renders the real honor code + a real device-auth login
// flow as terminal text/input before handing off to a normal shell, instead of a GUI panel app.
//
// Build (Linux):
//
//	sudo apt install libsdl2-dev
//	CGO_ENABLED=1 go build ./cmd/idunagame
//
...
//
//	CGO_ENABLED=1 go build -o idunagame.exe ./cmd/idunagame
//
...
import (
	...
	"idunagame/internal/font"
	"idunagame/internal/gfdapi"
	"idunagame/internal/mudconn"
	"idunagame/internal/pty"
	"idunagame/internal/scrollmod"
	"idunagame/internal/sshconn"
...
//
// IDUNA.GAME reskin (NORTHSTAR.md §1, founder real-time: "have it be a reskin of pitviper into
// solarized light") — real, canonical Solarized Light values (Ethan Schoonover's own published
// spec, https://ethanschoonover.com/solarized/), the same 16-slot terminal.sexy mapping virtually
// every terminal emulator's own "Solarized Light" preset uses. Not PITVIPER's own original dark
// ANSI palette, replaced wholesale, not tinted — a real reskin, not a filter over the old colors.
var baseColors = [16]sdl.Color{
	{R: 0x07, G: 0x36, B: 0x42, A: 0xff}, // 0 black    -> base02 #073642
	{R: 0xdc, G: 0x32, B: 0x2f, A: 0xff}, // 1 red      -> red     #dc322f
	{R: 0x85, G: 0x99, B: 0x00, A: 0xff}, // 2 green    -> green   #859900
	{R: 0xb5, G: 0x89, B: 0x00, A: 0xff}, // 3 yellow   -> yellow  #b58900
	{R: 0x26, G: 0x8b, B: 0xd2, A: 0xff}, // 4 blue     -> blue    #268bd2
	{R: 0xd3, G: 0x36, B: 0x82, A: 0xff}, // 5 magenta  -> magenta #d33682
	{R: 0x2a, G: 0xa1, B: 0x98, A: 0xff}, // 6 cyan     -> cyan    #2aa198
	{R: 0xee, G: 0xe8, B: 0xd5, A: 0xff}, // 7 white    -> base2   #eee8d5
	{R: 0x00, G: 0x2b, B: 0x36, A: 0xff}, // 8 bright black   -> base03 #002b36
	{R: 0xcb, G: 0x4b, B: 0x16, A: 0xff}, // 9 bright red     -> orange  #cb4b16
	{R: 0x58, G: 0x6e, B: 0x75, A: 0xff}, // 10 bright green  -> base01  #586e75
	{R: 0x65, G: 0x7b, B: 0x83, A: 0xff}, // 11 bright yellow -> base00  #657b83
	{R: 0x83, G: 0x94, B: 0x96, A: 0xff}, // 12 bright blue   -> base0   #839496
	{R: 0x6c, G: 0x71, B: 0xc4, A: 0xff}, // 13 bright magenta -> violet #6c71c4
	{R: 0x93, G: 0xa1, B: 0xa1, A: 0xff}, // 14 bright cyan   -> base1   #93a1a1
	{R: 0xfd, G: 0xf6, B: 0xe3, A: 0xff}, // 15 bright white  -> base3   #fdf6e3
}

// IDUNA brand palette, real Solarized Light values throughout — accent uses Solarized's own
// "yellow" (#b58900), the closest real Solarized swatch to IDUNA's own real gold (#b89b62,
// IDUNA/styles.css) without inventing an off-palette color (this reskin's whole point is to BE
// Solarized Light, not Solarized-tinted-toward-something-else).
var idunaPalette = struct {
	bg     sdl.Color // base3  #fdf6e3
	fg     sdl.Color // base00 #657b83
	accent sdl.Color // yellow #b58900 (IDUNA's own real gold's nearest Solarized swatch)
	freq   sdl.Color // cyan   #2aa198
	bloc   sdl.Color // red    #dc322f
	muted  sdl.Color // base1  #93a1a1
	barBG  sdl.Color // base2  #eee8d5
}{
	bg:     sdl.Color{R: 0xfd, G: 0xf6, B: 0xe3, A: 0xff},
	fg:     sdl.Color{R: 0x65, G: 0x7b, B: 0x83, A: 0xff},
	accent: sdl.Color{R: 0xb5, G: 0x89, B: 0x00, A: 0xff},
	freq:   sdl.Color{R: 0x2a, G: 0xa1, B: 0x98, A: 0xff},
	bloc:   sdl.Color{R: 0xdc, G: 0x32, B: 0x2f, A: 0xff},
	muted:  sdl.Color{R: 0x93, G: 0xa1, B: 0xa1, A: 0xff},
	barBG:  sdl.Color{R: 0xee, G: 0xe8, B: 0xd5, A: 0xff},
}

var defaultFG = sdl.Color{R: 0x65, G: 0x7b, B: 0x83, A: 0xff} // base00
var defaultBG = sdl.Color{R: 0xfd, G: 0xf6, B: 0xe3, A: 0xff} // base3
...
func main() {
	...
	if gfdMode {
		defaultBG = idunaPalette.bg
		defaultFG = idunaPalette.fg
	}
	...
	// IDUNA text-prompt sequence (NORTHSTAR.md §3, iduna_prompt.go) -- runs concurrently with the
	// real shell/SSH/MUD connection above so the terminal stays live and usable the whole time.
	go runIdunaPrompt(screen)
...
func renderGFDBar(ren *sdl.Renderer) {
	...
	_ = ren.SetDrawColor(idunaPalette.barBG.R, idunaPalette.barBG.G, idunaPalette.barBG.B, 0xff)
	_ = ren.FillRect(&sdl.Rect{X: 0, Y: barY, W: winW, H: barH})

	_ = ren.SetDrawColor(idunaPalette.accent.R, idunaPalette.accent.G, idunaPalette.accent.B, 0xff)
	...
	if gfdWebmaster && gfdClient != nil {
		...
		if state.TierName != "" {
			...
			renderBarText(ren, "["+state.TierName+"]", tierX, barY+2, idunaPalette.freq)
		}
	}
	...
	if tsX > 0 {
		renderBarText(ren, ts, tsX, barY+2, idunaPalette.muted)
	}
}
...
func renderDistrictPane(ren *sdl.Renderer, state gfdapi.State) {
	...
	muted := idunaPalette.muted
	freq := idunaPalette.freq
